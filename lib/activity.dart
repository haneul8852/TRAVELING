...
class _ActivityPageState extends State<ActivityPage> {
  ...
  @override
  Widget build(BuildContext context) {
    // 'cat'이 'activity'인 코스를 필터링
    List<Map<String, String>> currentActivities =
        courses.where((course) => course['cat'] == 'activity').toList();

    // 성향 기반 필터링
    List<Map<String, String>> filteredActivities = isUserPreferenceSelected
        ? currentActivities
            .where((activity) => activity['category'] == _travelPersonality)
            .toList()
        : currentActivities
            .where((activity) => activity['category'] != _travelPersonality)
            .toList();
    ...
    // 지역 기반 필터링
    filteredActivities = selectedRegion == '전체'
        ? filteredActivities
        : filteredActivities
            .where((activity) => activity['region'] == selectedRegion)
            .toList();
            ...
            child: ListView(
              children: filteredActivities.map((activity) {
                return ActivityItem(
                  ...
                  onTap: () {
                    if (activity['title'] == '제주도') {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => JejuScreen(),
                        ),
                      );
                    } else {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) =>
                              CourseDetailScreen(title: activity['title']!),
                        ),
                      );
                    }
                  },
