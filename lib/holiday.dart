...
class _HolidayPageState extends State<HolidayPage> {
  ...
  @override
  Widget build(BuildContext context) {
    List<Map<String, String>> currentHolidays =
        courses.where((course) => course['cat'] == 'holiday').toList();

    // 성향 기반 필터링
    List<Map<String, String>> filteredHolidays = isUserPreferenceSelected
        ? currentHolidays
            .where((holiday) => holiday['category'] == _travelPersonality)
            .toList()
        : currentHolidays
            .where((holiday) => holiday['category'] != _travelPersonality)
            .toList();
    ...
    // 지역 기반 필터링
    filteredHolidays = selectedRegion == '전체'
        ? filteredHolidays
        : filteredHolidays
            .where((holiday) => holiday['region'] == selectedRegion)
            .toList();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: ListView(
              children: filteredHolidays.map((holiday) {
                return HolidayItem(
                  title: holiday['title']!,
                  tags: holiday['tags']!,
                  ...
                  onTap: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            CourseDetailScreen(title: holiday['title']!),
                      ),
                    );
                  },
                );
